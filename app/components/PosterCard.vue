...
<template>
    <button type="button"
        ...
        <div class="poster relative w-full overflow-hidden rounded-lg transition-transform duration-200 will-change-transform group-hover:-translate-y-1 group-focus-visible:ring-2 group-focus-visible:ring-primary"
            ...
            <div class="absolute inset-x-2 bottom-2 text-[11px] leading-snug text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                <p v-if="item.requestedBy" class="truncate">
                    <span class="text-white/60">Requested</span> {{ item.requestedBy }}
                </p>
                <p v-if="item.watchedBy.length" class="truncate">
                    <span class="text-white/60">Watched</span> {{ item.watchedBy.join(', ') }}
                </p>
                <p v-if="item.lastPlayed" class="truncate text-white/60">
                    {{ idleDays(item.lastPlayed) }}d idle
                </p>
            </div>

            <div v-if="item.status !== 'keep'" class="absolute top-1.5 left-1.5">
                <StatusBadge :status="item.status" size="xs" />
            </div>
            <UIcon v-else-if="item.lastPlayed" name="i-lucide-check" class="absolute top-1.5 right-1.5 size-4 rounded-full bg-success p-0.5 text-inverted shadow" />
        </div>

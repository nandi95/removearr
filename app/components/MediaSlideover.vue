...
watch(item, value => {
    if (!value && selected.value) selected.value = null;
});
</script>

<template>
    <USlideover v-model:open="open"
                :ui="{ content: 'max-w-lg', body: 'p-0 sm:p-0' }"
                :title="item?.title ?? ''"
                :description="item ? `${item.year} · ${gb(item.size)}` : ''">
        <template #content>
            <div v-if="item && selected" class="relative flex h-full flex-col overflow-y-auto">
                <div class="relative h-56 shrink-0 overflow-hidden">
                    <img :src="`/api/art/${selected.kind}/${item.id}/backdrop`"
                         alt=""
                         class="size-full scale-110 object-cover blur-sm brightness-50"
                         @error="($event.target as HTMLImageElement).style.display = 'none'">
                    <div class="absolute inset-0 bg-linear-to-t from-default via-default/40 to-transparent" />
                    <UButton icon="i-lucide-x"
                             color="neutral"
                             variant="ghost"
                             class="absolute top-3 right-3 bg-black/30 text-white hover:bg-black/50"
                             @click="open = false" />
                </div>
                ...
                <div class="relative z-10 -mt-28 flex gap-5 px-6">
                    ...
                    <div class="mt-16 min-w-0 self-end pb-1">
                        <h2 class="text-xl leading-tight font-semibold tracking-tight">
                            {{ item.title }}
                        </h2>
                        <p class="mt-1 text-sm text-muted">
                            {{ item.year }} · {{ gb(item.size) }}<template v-if="show">
                                · {{ plural(show.seasons.length, 'season') }}
                            </template>
                        </p>
                        <div class="mt-2">
                ...
                <dl class="mt-6 grid grid-cols-3 gap-4 border-y border-default px-6 py-4 text-sm">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 truncate font-medium" :title="item.watchedBy.join(', ')">
                            {{ item.watchedBy.join(', ') || 'Nobody' }}
                        </dd>
                    </div>
                    <div>
                        <dt class="text-xs tracking-wide text-dimmed uppercase">
                        ...
                        <dd class="mt-1 truncate font-medium">
                            <template v-if="item.lastPlayed">
                                {{ shortDate(item.lastPlayed) }} <span class="text-muted">· {{ idle }}d</span>
                            </template>
                            <template v-else>
                                Never
                            </template>
                ...
                <div v-if="movie" class="flex items-center gap-2 px-6 py-5">
                    <DeleteButton :candidate="candidates[0]!"
                                  size="md"
                                  class="flex-1"
                                  block />
                ...
                <ul v-else class="divide-y divide-default p-2">
                    <li v-for="c in candidates" :key="c.key" class="flex items-center gap-3 rounded-lg px-4 py-3 hover:bg-elevated/50">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-0.5 truncate text-xs text-muted">
                                ...
                                <template v-if="c.watchedBy.length">
                                    · watched by {{ c.watchedBy.join(', ') }}
                                </template>
                                <template v-if="c.lastActivity">

...
<template>
    <UModal v-model:open="open"
            :dismissible="!running"
            :title="finished ? 'Done' : `Delete ${plural(list.length, 'item')}?`"
            :description="finished ? undefined : `${gb(total)} will be removed from disk through Radarr and Sonarr. This cannot be undone.`">
        <template #body>
            <ul class="max-h-72 divide-y divide-default overflow-y-auto rounded-lg border border-default text-sm">
                <li v-for="c in list" :key="c.key" class="flex items-center gap-3 px-3 py-2">
                    ...
                    <span class="shrink-0 text-muted tabular-nums">{{ gb(c.size) }}</span>
                </li>
            </ul>
            <UProgress v-if="running || finished"
                       :value="done"
                       :max="list.length"
                       class="mt-4"

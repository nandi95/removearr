        ...
        <div class="min-w-0 flex-1 text-sm">
            <template v-if="event.type === 'deletion'">
                ...
                <p class="text-xs text-muted">
                    {{ gb(event.deletion.sizeBytes) }} freed<template v-if="event.deletion.requestedBy">
                        · requested by {{ event.deletion.requestedBy }}
                    </template>
                </p>
            </template>
            <template v-else-if="event.run.ok">
                ...
                <p class="text-xs text-muted">
                    {{ gb(event.run.deletableBytes) }} reclaimable · Plex +{{ event.run.plexAdded }} −{{ event.run.plexRemoved }}
                </p>
            </template>
            <template v-else>
                <p class="text-error">
                ...
                <p class="truncate text-xs text-muted" :title="event.run.error ?? ''">
                    {{ event.run.error }}
                </p>
            </template>
        </div>

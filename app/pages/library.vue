  ...
  <UDashboardPanel id="library">
      ...
      <UDashboardToolbar>
        <template #left>
          <UTabs
            ...
            variant="link"
          />
          <UInput v-model="query" icon="i-lucide-search" placeholder="Search titles…" size="sm" class="w-48" :ui="{ trailing: 'pe-1' }">
            <template v-if="query" #trailing>
              <UButton icon="i-lucide-x" color="neutral" variant="link" size="xs" aria-label="Clear" @click="query = ''" />
            </template>
          </UInput>
        </template>
        <template #right>
          ...
          />
          <USelect v-model="requester" :items="requesters" size="sm" class="w-36" icon="i-lucide-user" />
          <USelect
            v-model="sort"
            ...
            class="w-36"
          />
          <UButton :icon="desc ? 'i-lucide-arrow-down-wide-narrow' : 'i-lucide-arrow-up-narrow-wide'" color="neutral" variant="subtle" size="sm" :aria-label="desc ? 'Descending' : 'Ascending'" @click="desc = !desc" />
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <div v-else class="grid grid-cols-[repeat(auto-fill,minmax(160px,1fr))] gap-5">
        <PosterCard v-for="item in items" :key="item.id" :kind="kind" :item="item" />
      </div>
    </template>
  </UDashboardPanel>
</template>

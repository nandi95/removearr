  ...
  <UDashboardPanel id="overview">
    <template #header>
      <UDashboardNavbar title="Overview">
        ...
        <template #right>
          ...
          <UButton label="Sync now" icon="i-lucide-refresh-cw" :loading="syncing" color="neutral" variant="subtle" @click="sync" />
        </template>
      </UDashboardNavbar>
    </template>
    ...
    <template #body>
      <div class="grid grid-cols-2 gap-4 xl:grid-cols-4">
        ...
        <template v-else>
          <StatTile label="Reclaimable now" :value="gb(sum(totals.deletable))" :hint="plural(totals.deletable.length, 'item') + ' ready to delete'" icon="i-lucide-trash-2" color="error" to="/review" />
          <StatTile label="Leaving soon" :value="gb(sum(totals.soon))" :hint="plural(totals.soon.length, 'item') + ` · deletable after ${data?.deleteAfterDays ?? 14} idle days`" icon="i-lucide-hourglass" color="warning" to="/review?tab=soon" />
          <StatTile label="Library on disk" :value="gb(totals.libraryBytes, 0)" :hint="`${plural(movies.length, 'movie')} · ${plural(series.length, 'series')}`" icon="i-lucide-hard-drive" color="primary" to="/library" />
          <StatTile label="Freed all time" :value="gb(activity?.freedBytes ?? 0)" :hint="`${plural(totals.snoozed.length, 'item')} kept`" icon="i-lucide-sparkles" color="success" to="/activity" />
        </template>
      </div>
      ...
      <div class="mt-6 grid gap-6 lg:grid-cols-3">
        <section class="lg:col-span-2">
          <div class="mb-3 flex items-baseline justify-between">
            ...
            <UButton to="/review" label="Review all" trailing-icon="i-lucide-arrow-right" variant="link" size="xs" color="neutral" />
          </div>
          ...
          />
          <div v-else class="scrollbar-none -mx-1 flex snap-x gap-4 overflow-x-auto px-1 pb-2">
            <PosterCard v-for="c in upNext" :key="c.key" :kind="c.kind" :item="posterFor(c)" size="sm" />
          </div>

          <div v-if="trend.length > 1" class="mt-6 rounded-xl border border-default bg-elevated/40 p-5">
            <div class="flex items-baseline justify-between">
              <h2 class="text-sm font-semibold">Reclaimable over time</h2>
        ...
        <section>
          <div class="mb-3 flex items-baseline justify-between">
            ...
            <UButton to="/activity" label="All" trailing-icon="i-lucide-arrow-right" variant="link" size="xs" color="neutral" />
          </div>
          <div class="rounded-xl border border-default bg-elevated/40 p-5">
            ...
            <p v-else-if="!activity.events.length" class="text-sm text-muted">No syncs or deletions yet.</p>
            <ul v-else class="space-y-4">
              <li v-for="(event, i) in activity.events" :key="i"><ActivityItem :event="event" /></li>
            </ul>
          </div>
        </section>
      </div>

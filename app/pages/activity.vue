    ...
    <template #body>
      ...
      <div v-else class="mx-auto max-w-2xl space-y-8">
        <section v-for="[day, list] in days" :key="day">
          <h2 class="mb-3 text-xs font-semibold uppercase tracking-wide text-dimmed">{{ dayLabel(day) }}</h2>
          <ul class="divide-y divide-default rounded-xl border border-default bg-elevated/40">
            <li v-for="(event, i) in list" :key="i" class="px-4 py-3"><ActivityItem :event="event" /></li>
          </ul>
        </section>
      </div>
    </template>
